...
select ulke, count(ulke) as toplam_calisan from personel group by ulke;
...
select ulke, sehir from personel group by sehir,ulke;



-- SORU06: Personelin, ulkelere göre ve şehirler göre calışan sayısını sorgulayın.
select ulke,cinsiyet, count(cinsiyet)  , avg(yas) from personel group by ulke, cinsiyet;



-- SORU07: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını sorgulayınız.
select cinsiyet,ulke, count(ad)as calsian_sayisi, round(avg(yas)) as ort_yas from personel group by ulke,cinsiyet;



-- SORU08: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını  ve maası 30000 den büyük olanları sorgulayınız.
select ulke, cinsiyet, count(cinsiyet) , avg(yas) , maas
from personel
where maas>30000
...
select ulke,cinsiyet,count(cinsiyet) ,round(avg(yas))from personel  where maas>30000 group by ulke,cinsiyet order by  yas desc;
...
select isim, urun_adi, sum(urun_miktar) as tablom_urun
from manav
...
-- aggrate fonksiyon kullandiysak where yerine having kullanacagiz
...
select urun_adi,max(urun_miktari)as max_miktar from manav where urun_adi is not null group by urun_adi order by max_miktar desc;
...
select urun_adi, sum(urun_miktari) from manav where urun_adi is not null group by urun_adi;
